...
declare
	l_avoir client.avoir%type;
begin
	SELECT avoir into l_avoir
	FROM Client WHERE idc = :old.idc;

	INSERT INTO Archive values(:old.ids, :old.idc, :old.idv, :old.jour, l_avoir);
	dbms_output.put_line('Archivage Sejour : ('||:old.ids||', '||:old.idc||', '||:old.idv||', '||:old.jour||', '||l_avoir||')');
end;
/
...
*/
drop trigger contrainteAvoir;

create or replace trigger contrainteAvoir
	after INSERT
	on Sejour
declare
	cursor c is
...
begin
	...
	if c%found then
		raise_application_error(-20001, 'violation contrainteAvoir pour '||le_client.nom||', avoir = '||le_client.avoir||', depenses = '||le_client.depenses);
	end if;
end;
/
...
-- achat séjour à Londres par Bob
declare
	l_idv 	   Village.idv%type;
